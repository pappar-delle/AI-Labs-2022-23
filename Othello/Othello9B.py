...
def main():
    ...
    if movesLeft > 0 and len(moveSet) > 0:
        ...
        if movesLeft < holeLim:
            ab = alphabetaMain(board, token, -65, 65, dotPos)
        else:
            if movePref(moveSet, board, token, dotPos) not in cnrS:
                midgame(board, token, -100000, 100000, 2, dotPos)
...
def quickMove(board, token, dP):
    if not board:
        ...
        board = board.upper()
        token = token.upper()
        possMoveSet = findMoves(board, token, dP)
        numHoles = board.count('.')
        if numHoles < holeLim:
            ab = alphabetaMain(board, token, -65, 65, dP)
            return ab[-1]
        return movePref(possMoveSet, board, token, dP)
...
def heuristic(board, token, currMove, oppMove, scoreCnr, scoreFill, normScore, varScore):
    ...
    return  382.026*scoreFill+ 801.724*scoreCnr + 120*normScore + 78.922*varScore
...
class Strategy:
    ...
    def best_strategy(self, board, player, best_move, running):
        dotPos = {i for i in range(64) if board[i] == "."}
        if running.value:
            best_move.value = quickMove(board, player, dotPos)
